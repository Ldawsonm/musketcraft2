...
### Mobs
    ...
    #### Oozer
    execute as @e[type=item,nbt={Item:{id:"minecraft:rotten_flesh",Count:1b,tag:{Tags:["oozer_death"]}}}] at @s positioned ~ ~1 ~ run function cavernous:myst/mobs/oozer/explode

    #### Ghost
    execute as @e[type=skeleton,tag=ghost] at @s if entity @a[distance=..40] run function cavernous:myst/mobs/ghost/run
...
execute as @e[type=skeleton,tag=priest_2] at @s run function cavernous:myst/arena/priest_run_2
    ...
    execute as @e[scores={return_portal.to_be_tped_to_ws=1}] run tp @s ~ ~ ~
